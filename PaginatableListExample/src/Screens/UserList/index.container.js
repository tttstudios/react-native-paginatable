...
class UserListContainer extends Component {
	...
	onHighlightUser = index => {
		this.props.dispatch(
			customizedPaginationStateManager.highlightItem({
				index: index,
				extra: 'extra'
			})
		)
	...
	onLoadMoreUsers = ({ ...args }, onCompleteLoadMore, onLoadError) => {
		this.props.dispatch(
			customizedPaginationStateManager.loadMore(
				{ ...args, keyword: '123', isValid: true },
				onCompleteLoadMore,
				onLoadError
			)
		)
	...
	onRefreshUserList = ({ ...args }, onCompleteRefreshing, onLoadError) => {
		this.props.dispatch(
			customizedPaginationStateManager.refresh(
				{ ...args, keyword: '123', isValid: true },
	...
	onCompleteLoadMore = () => {
		if (__DEV__)
			console.log('Customized completion handler of loading more items.')
	}

	onCompleteRefresh = () => {
		if (__DEV__)
			console.log('Customized completion handler of refreshing the list.')
	}
